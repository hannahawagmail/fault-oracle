...
def query_range(prometheus_url: str, query: str, window: str) -> dict:
    """
...
def query_instant(prometheus_url: str, query: str) -> dict:
...
def _label_key(metric: dict) -> str:
    """Build a deterministic Prometheus label string from a metric dict.

    Sorting by key ensures the same label set always produces the same string,
    which is required for matching range-query series against instant-query
    series by dict lookup in run().
    """
    return ",".join(f'{k}="{v}"' for k, v in sorted(metric.items()))


def _window_to_seconds(window: str) -> float:
    """Convert a Prometheus-style duration string (e.g. '7d', '5m') to seconds."""
...
def compute_zscore(current: float, mean: float, stddev: float) -> float:
    """Compute the Z-score of current relative to the baseline distribution.

    Z = (x - mean) / stddev. stddev must be non-zero; callers should use
    compute_stats() which clamps stddev to 1.0 for constant/empty series
    to prevent a ZeroDivisionError here.
    """
    return (current - mean) / stddev


def emit_metrics(zscores: dict, threshold: float) -> str:
    """Render Prometheus textfile exposition for all computed Z-scores.

    Each label_set gets one sample in anomaly_zscore and one in anomaly_detected.
    The threshold (default 3.0) represents ≈0.3% false-positive rate under a
    Gaussian distribution — chosen to balance alert noise vs. missed detections
    for EDAC CE rate, which is roughly Poisson-distributed at low rates.

    Args:
        zscores: {label_str: {zscore, mean, stddev, stddev_clamped, current}}
        threshold: absolute Z-score above which anomaly_detected=1

    Returns:
        Complete Prometheus text exposition string ending with a newline.
    """
    lines = []
    now = time.time()
...
def run(prometheus_url: str, query: str, window: str, threshold: float) -> dict:
...
def main():
    """Entry point: query Prometheus, compute Z-scores, write textfile metrics.

    Exits with code 1 if the Prometheus query fails so that a systemd timer
    can detect failures and alert via OnFailure= or a watchdog timeout.
    """

...
class HardwareEvent:
    ...
    pci_device: Optional[str]
    aer_error_type: Optional[str]
    raw_message: str
...
RE_AER_CORRECTED = re.compile(
    r'AER:\s+Corrected error received:\s+([\da-fA-F:.]+)'
)
...
class TraceParser:
    ...
    def _parse_line(self, line: str, all_lines: list, line_idx: int) -> Optional[HardwareEvent]:
        # Extract timestamp
    ...
    def _look_ahead_aer_type(self, lines: list, idx: int, lookahead: int = 5) -> Optional[str]:
    ...
    def _make_edac_event(self, event_type: str, m, clean: str, raw: str,
                         monotonic_s, timestamp_str) -> HardwareEvent:
    ...
    def _make_aer_event(self, event_type: str, device: str, aer_type: Optional[str],
                        raw: str, monotonic_s, timestamp_str) -> HardwareEvent:
        ev = HardwareEvent(
            seq=self.seq,
            timestamp_str=timestamp_str,
            timestamp_ns=int(monotonic_s * 1e9) if monotonic_s else 0,
    ...
    if events:
        ...
        if first_mono and last_mono:
            ...
            print(f"Time span: {span:.3f}s "
                  f"({first_mono:.3f}s → {last_mono:.3f}s monotonic)", file=sys.stderr)
...
def main():
    ...
    parser.add_argument('--input', '-i', default='-',
                        help='Input log file (- for stdin, default: -)')
    parser.add_argument('--output', '-o', default='-',
                        help='Output JSON file (- for stdout, default: -)')
    parser.add_argument('--event-types', default='CE,UE,AER_CE,AER_UE,MCE',

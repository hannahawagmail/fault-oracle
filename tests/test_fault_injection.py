...
def run_script(script: str, args: list[str], env=None, input_text=None) -> subprocess.CompletedProcess:
...
class TestArgumentValidation:
    ...
    def test_unknown_option_rejected(self):
        ...
        for script in ["inject_edac_ce.sh", "inject_edac_ue.sh"]:
            ...
            assert result.returncode != 0, (
                f"{script} should reject unknown options"
            )

    @pytest.mark.skipif(not has_root() or not has_edac_module(),
                        reason="requires root + loaded module")
...
class TestFaultMatrix:
    ...
    def test_unknown_fault_type_skipped(self):
        result = run_script("ci_fault_matrix.sh",
                            ["--dry-run", "--types", "INVALID_TYPE"])
...
class TestCounterDelta:
    ...
    @pytest.mark.parametrize("baseline,new_val,count,should_pass", [
...
class TestAERBackendDetection:
    ...
    def test_no_backend_exits_2(self):
        """When neither aer-inject nor EINJ is available, script must exit 2 (skip)."""
        if has_root() and Path("/dev/aer-inject").exists():
            pytest.skip("aer-inject is available on this system")
        try:
            einj_exists = Path("/sys/kernel/debug/apei/einj/error_inject").exists()
        except PermissionError:
            einj_exists = False
        if einj_exists:
            pytest.skip("ACPI EINJ is available on this system")

        # Run with --backend none explicitly
        result = run_script("inject_aer.sh", ["--backend", "none"])
        assert result.returncode == 2, (
            f"Expected exit 2 (skip) for missing backend, got {result.returncode}"
        )
    ...
    def test_error_type_flag_accepted(self, error_type):
        result = run_script("inject_aer.sh", [
            "--type", error_type,
            "--no-verify",
        ])
...
class TestInjectionIntegration:
    ...
    @pytest.mark.skipif(not has_root() or not has_edac_module(),
                        reason="requires root + edac_cortex_ref module")
    ...
    def test_ue_injection_increments_counter(self):
        result = run_script("inject_edac_ue.sh", [
            "--controller", "mc0",
            "--csrow", "0",
            "--channel", "0",
            "--count", "1",

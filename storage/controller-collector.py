...
"""
storage/controller-collector.py — Collect RAID/HBA controller error metrics.

Detects RAID/HBA controllers via sysfs driver directories, queries storcli
for MegaRAID controllers, and emits Prometheus textfile metrics.

Degrades gracefully when no controllers are present or storcli is absent.

Usage:
    python3 storage/controller-collector.py [--output PATH] [--sys-pci-path PATH]

Writes to: /var/lib/node_exporter/textfile_collector/controller.prom (default)

Metrics:
    storage_controller_up{driver}                              — 1 if driver found
    storage_controller_temperature_celsius{controller,driver}  — controller temperature
    storage_controller_critical_disks{controller,driver}       — critical disk count
    storage_controller_failed_disks{controller,driver}         — failed disk count
    storage_controller_ecc_errors_total{controller,driver,error_type} — ECC error counts
    storage_controller_last_run_timestamp                      — Unix timestamp of last run
"""
    ...
    for driver in ALL_DRIVERS:
        ...
        try:
            if not driver_path.exists():
                continue
            # Non-empty means it has at least one PCI device bound
...
def main():
    ...
    if not detected_drivers:
        sys.stderr.write(
            "controller-collector: no RAID/HBA controllers detected, emitting controller_up{driver=\"none\"} 0\n"
        )

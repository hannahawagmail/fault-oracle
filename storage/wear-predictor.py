...
"""
storage/wear-predictor.py — NVMe and SATA wear-out failure predictor.

Queries Prometheus for wear percentage metrics over 30 days, fits an OLS
linear regression per device, and emits Prometheus textfile metrics with
projected time-to-failure.

Degrades gracefully when Prometheus is unreachable or insufficient history
is available.

Usage:
    python3 storage/wear-predictor.py [--output PATH] [--prometheus-url URL]

Writes to: /var/lib/node_exporter/textfile_collector/wear.prom (default)

Metrics:
    storage_wear_time_to_failure_days{device,type}   — projected days to 100% wear
    storage_wear_rate_percent_per_day{device,type}   — wear rate per day
    storage_wear_probability_90d{device,type}        — 1.0 if failure within 90 days
    storage_wear_predictor_insufficient_data{device} — 1 if < 72h history
    storage_wear_predictor_up                        — 1 if predictor ran successfully
    storage_wear_predictor_last_run_timestamp        — Unix timestamp of last run
"""
...
def query_prometheus_range(base_url: str, query: str, start: float, end: float, step: int):
    """

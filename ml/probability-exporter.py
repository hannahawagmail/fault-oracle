...
def log_error(msg: str):
    """Log an error to both the module logger and stderr.

    Writing to stderr ensures the message appears in systemd journal output
    even when the logger is misconfigured, which matters for this script that
    runs unattended as a systemd timer.
    """
    log.error(msg)
...
def risk_tier(prob: float) -> str:
    """Map a failure probability to a human-readable risk tier label.

    Tiers are evaluated in descending threshold order (critical → low) so the
    first match wins. The sentinel (0.0, 'low') ensures a value is always
    returned without an explicit else branch.
    """
...
def load_metadata(model_dir: Path) -> list:
    """Load all model metadata JSON files from model_dir and return a list of dicts.

    Each JSON file was written by ce-forecaster.py and contains the label_key,
    failure probabilities, model path, and training timestamp needed to emit
    metrics without re-loading the (potentially large) pickle files.
    """
    return [json.loads(p.read_text()) for p in model_dir.glob("*.json")]


def forecast_next_24h(model_path: str, label_key: str) -> float:
...
def emit_metrics(entries: list, now: float) -> str:
    """Render Prometheus textfile exposition for all loaded DIMM model entries.

    Produces the complete .prom file content including # HELP / # TYPE headers
    and one sample line per (label_key, tier) combination. The forecast_next_24h
    metric requires loading the pickle model from disk, so this function may be
    slow if many models are stale or the disk is busy.

    Args:
        entries: list of metadata dicts loaded from model_dir/*.json
        now: current Unix timestamp (passed in so callers can fix it for testing)

    Returns:
        String in Prometheus text exposition format, ending with a newline.
    """
    lines = []
...
def main():
    """Entry point: load model metadata, emit metrics to textfile collector output.

    When model_dir does not exist (e.g. ce-forecaster has never run), emits a
    stub ml_models_loaded_total=0 so node_exporter does not show a gap in the
    metric instead of producing an error.
    """
